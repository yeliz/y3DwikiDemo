package
{
	...
	import com.yogurt3d.core.materials.MaterialTwoTextureFresnel;
	...
	public class wikiTwoTexture extends BaseTest
	{
		...
		private function createUI():void{
			...
			window = new Window(this,m_resourceManager.windowX, m_resourceManager.windowY,"TwoTexture Properties");
			...
			normalCheckBox   = new CheckBox(window, 5, 5, "Normal map",function(_e:Event):void{
				if(MaterialTwoTextureFresnel(m_sceneObject.material).normalMap == null){
					MaterialTwoTextureFresnel(m_sceneObject.material).normalMap = getMap("normalMap");
				}else{
					MaterialTwoTextureFresnel(m_sceneObject.material).normalMap = null;
				}
			});
			reflectivityCheckBox = new CheckBox(window, 5, 15, "Reflectivity map",function(_e:Event):void{
				if(MaterialTwoTextureFresnel(m_sceneObject.material).reflectivityMap == null){
					MaterialTwoTextureFresnel(m_sceneObject.material).reflectivityMap = getMap("reflectionMap");
				}else{
					MaterialTwoTextureFresnel(m_sceneObject.material).reflectivityMap = null;
				}
			});
			...
			alphaSlider = new HSlider(window, 5, 45, function(_e:Event):void{
				...
				MaterialTwoTextureFresnel(m_sceneObject.material).opacity = alphaSlider.value;
			});
			...
			fresnelSlider = new HSlider(window, 5, 70,function(_e:Event):void{
				MaterialTwoTextureFresnel(m_sceneObject.material).fresnelReflectance = fresnelSlider.value;
				fresnelLabel.text = "Fresnel Reflectance: "+ fresnelSlider.value;
			});
			fresnelSlider.maximum = 1;
			fresnelSlider.minimum = 0;
			fresnelSlider.value = MaterialTwoTextureFresnel(m_sceneObject.material).fresnelReflectance;	
			fresnelLabel.text = "Fresnel Reflectance: "+ fresnelSlider.value;
			
			powerLabel = new Label(window, 5, 80,"Fresnel Power");
			fresnelPowerSlider = new HSlider(window, 5, 95, function(_e:Event):void{
				MaterialTwoTextureFresnel(m_sceneObject.material).fresnelPower = uint(fresnelPowerSlider.value);
			...
			gainSlider = new HSlider(window, 5, 120, function(_e:Event):void{
				...
				MaterialTwoTextureFresnel(m_sceneObject.material).gain = gainSlider.value;
			});
